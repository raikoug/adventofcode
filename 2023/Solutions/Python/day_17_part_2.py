...
while QUEUE:
    ...
    for i,[dir_col, dir_row] in enumerate(N):
        ...
        # checking is we are inside the grid and 
        #  if we are not going straight 10 times in a row, but at least 4
        #   and if we are not going back (that we cannot!)
        if all([
            0<=new_row<MAX_Row,
            0<=new_col<MAX_Col,
            new_cons_dir<=10,             # no more than 10 straight moves
            ((new_dir+2)%4) != direction, # not going back
            (   new_dir == direction or   # going straight
                cons_dir >= 4        or   # at least 4 straight moves
                cons_dir == -1            # first move (initial D changed accordingly)
            )
         ]):
